...
insertRule_index domcssMediaRule  rule index =
  withObjCPtr rule $ \raw_rule ->
      sendMsg domcssMediaRule (mkSelector "insertRule:index:") retCUInt [argPtr (castPtr raw_rule :: Ptr ()), argCUInt index]

-- | @- deleteRule:@
deleteRule :: IsDOMCSSMediaRule domcssMediaRule => domcssMediaRule -> CUInt -> IO ()
deleteRule domcssMediaRule  index =
    sendMsg domcssMediaRule (mkSelector "deleteRule:") retVoid [argCUInt index]
...
insertRule domcssMediaRule  rule index =
  withObjCPtr rule $ \raw_rule ->
      sendMsg domcssMediaRule (mkSelector "insertRule::") retCUInt [argPtr (castPtr raw_rule :: Ptr ()), argCUInt index]
