...
ruleWithPredicate_assertingFact_grade predicate fact grade =
  do
    ...
    withObjCPtr predicate $ \raw_predicate ->
      sendClassMsg cls' (mkSelector "ruleWithPredicate:assertingFact:grade:") (retPtr retVoid) [argPtr (castPtr raw_predicate :: Ptr ()), argPtr (castPtr (unRawId fact) :: Ptr ()), argCFloat grade] >>= retainedObject . castPtr
...
ruleWithPredicate_retractingFact_grade predicate fact grade =
  do
    cls' <- getRequiredClass "GKRule"
    withObjCPtr predicate $ \raw_predicate ->
      sendClassMsg cls' (mkSelector "ruleWithPredicate:retractingFact:grade:") (retPtr retVoid) [argPtr (castPtr raw_predicate :: Ptr ()), argPtr (castPtr (unRawId fact) :: Ptr ()), argCFloat grade] >>= retainedObject . castPtr
...
setSalience gkRule  value =
    sendMsg gkRule (mkSelector "setSalience:") retVoid [argCLong value]
