...
createIndicator_min_max_currentValue ioBluetoothHandsFreeAudioGateway  indicatorName minValue maxValue currentValue =
  withObjCPtr indicatorName $ \raw_indicatorName ->
      sendMsg ioBluetoothHandsFreeAudioGateway (mkSelector "createIndicator:min:max:currentValue:") retVoid [argPtr (castPtr raw_indicatorName :: Ptr ()), argCInt minValue, argCInt maxValue, argCInt currentValue]
