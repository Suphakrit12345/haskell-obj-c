...
setSelected_atIndex nsToolbarItemGroup  selected index =
    sendMsg nsToolbarItemGroup (mkSelector "setSelected:atIndex:") retVoid [argCULong (if selected then 1 else 0), argCLong index]
...
isSelectedAtIndex nsToolbarItemGroup  index =
    fmap ((/= 0) :: CULong -> Bool) $ sendMsg nsToolbarItemGroup (mkSelector "isSelectedAtIndex:") retCULong [argCLong index]
...
setSelectedIndex nsToolbarItemGroup  value =
    sendMsg nsToolbarItemGroup (mkSelector "setSelectedIndex:") retVoid [argCLong value]
