...
drawInRect_angle nsGradient  rect angle =
    sendMsg nsGradient (mkSelector "drawInRect:angle:") retVoid [argNSRect rect, argCDouble angle]
...
drawInBezierPath_angle nsGradient  path angle =
  withObjCPtr path $ \raw_path ->
      sendMsg nsGradient (mkSelector "drawInBezierPath:angle:") retVoid [argPtr (castPtr raw_path :: Ptr ()), argCDouble angle]
...
drawFromCenter_radius_toCenter_radius_options nsGradient  startCenter startRadius endCenter endRadius options =
    sendMsg nsGradient (mkSelector "drawFromCenter:radius:toCenter:radius:options:") retVoid [argNSPoint startCenter, argCDouble startRadius, argNSPoint endCenter, argCDouble endRadius, argCULong (coerce options)]
...
getColor_location_atIndex nsGradient  color location index =
  withObjCPtr color $ \raw_color ->
      sendMsg nsGradient (mkSelector "getColor:location:atIndex:") retVoid [argPtr (castPtr raw_color :: Ptr ()), argPtr location, argCLong index]
...
interpolatedColorAtLocation nsGradient  location =
    sendMsg nsGradient (mkSelector "interpolatedColorAtLocation:") (retPtr retVoid) [argCDouble location] >>= retainedObject . castPtr
