...
glyphInfoWithGlyph_forFont_baseString glyph font string =
  do
    ...
    withObjCPtr font $ \raw_font ->
      withObjCPtr string $ \raw_string ->
        sendClassMsg cls' (mkSelector "glyphInfoWithGlyph:forFont:baseString:") (retPtr retVoid) [argCUInt glyph, argPtr (castPtr raw_font :: Ptr ()), argPtr (castPtr raw_string :: Ptr ())] >>= retainedObject . castPtr
...
glyphInfoWithCharacterIdentifier_collection_baseString cid characterCollection string =
  do
    cls' <- getRequiredClass "NSGlyphInfo"
    withObjCPtr string $ \raw_string ->
      sendClassMsg cls' (mkSelector "glyphInfoWithCharacterIdentifier:collection:baseString:") (retPtr retVoid) [argCULong cid, argCULong (coerce characterCollection), argPtr (castPtr raw_string :: Ptr ())] >>= retainedObject . castPtr
