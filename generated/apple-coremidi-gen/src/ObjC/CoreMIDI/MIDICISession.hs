...
profileStateForChannel midiciSession  channel =
    sendMsg midiciSession (mkSelector "profileStateForChannel:") (retPtr retVoid) [argCUChar channel] >>= retainedObject . castPtr
...
enableProfile_onChannel_error midiciSession  profile channel outError =
  withObjCPtr profile $ \raw_profile ->
    withObjCPtr outError $ \raw_outError ->
        fmap ((/= 0) :: CULong -> Bool) $ sendMsg midiciSession (mkSelector "enableProfile:onChannel:error:") retCULong [argPtr (castPtr raw_profile :: Ptr ()), argCUChar channel, argPtr (castPtr raw_outError :: Ptr ())]
...
disableProfile_onChannel_error midiciSession  profile channel outError =
  withObjCPtr profile $ \raw_profile ->
    withObjCPtr outError $ \raw_outError ->
        fmap ((/= 0) :: CULong -> Bool) $ sendMsg midiciSession (mkSelector "disableProfile:onChannel:error:") retCULong [argPtr (castPtr raw_profile :: Ptr ()), argCUChar channel, argPtr (castPtr raw_outError :: Ptr ())]
...
sendProfile_onChannel_profileData midiciSession  profile channel profileSpecificData =
  withObjCPtr profile $ \raw_profile ->
    withObjCPtr profileSpecificData $ \raw_profileSpecificData ->
        fmap ((/= 0) :: CULong -> Bool) $ sendMsg midiciSession (mkSelector "sendProfile:onChannel:profileData:") retCULong [argPtr (castPtr raw_profile :: Ptr ()), argCUChar channel, argPtr (castPtr raw_profileSpecificData :: Ptr ())]
