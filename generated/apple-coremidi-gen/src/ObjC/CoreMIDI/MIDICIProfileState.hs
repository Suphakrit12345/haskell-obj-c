...
initWithChannel_enabledProfiles_disabledProfiles midiciProfileState  midiChannelNum enabled disabled =
  withObjCPtr enabled $ \raw_enabled ->
    withObjCPtr disabled $ \raw_disabled ->
        sendMsg midiciProfileState (mkSelector "initWithChannel:enabledProfiles:disabledProfiles:") (retPtr retVoid) [argCUChar midiChannelNum, argPtr (castPtr raw_enabled :: Ptr ()), argPtr (castPtr raw_disabled :: Ptr ())] >>= ownedObject . castPtr
