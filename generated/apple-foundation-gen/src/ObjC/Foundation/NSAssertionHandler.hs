...
handleFailureInMethod_object_file_lineNumber_description nsAssertionHandler  selector object fileName line format =
  withObjCPtr fileName $ \raw_fileName ->
    withObjCPtr format $ \raw_format ->
        sendMsg nsAssertionHandler (mkSelector "handleFailureInMethod:object:file:lineNumber:description:") retVoid [argPtr (unSelector selector), argPtr (castPtr (unRawId object) :: Ptr ()), argPtr (castPtr raw_fileName :: Ptr ()), argCLong line, argPtr (castPtr raw_format :: Ptr ())]
...
handleFailureInFunction_file_lineNumber_description nsAssertionHandler  functionName fileName line format =
  withObjCPtr functionName $ \raw_functionName ->
    withObjCPtr fileName $ \raw_fileName ->
      withObjCPtr format $ \raw_format ->
          sendMsg nsAssertionHandler (mkSelector "handleFailureInFunction:file:lineNumber:description:") retVoid [argPtr (castPtr raw_functionName :: Ptr ()), argPtr (castPtr raw_fileName :: Ptr ()), argCLong line, argPtr (castPtr raw_format :: Ptr ())]
