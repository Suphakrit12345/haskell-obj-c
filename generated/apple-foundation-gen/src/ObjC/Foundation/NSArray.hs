...
objectAtIndex nsArray  index =
    fmap (RawId . castPtr) $ sendMsg nsArray (mkSelector "objectAtIndex:") (retPtr retVoid) [argCULong index]
...
initWithObjects_count nsArray  objects cnt =
    sendMsg nsArray (mkSelector "initWithObjects:count:") (retPtr retVoid) [argPtr (castPtr (unRawId objects) :: Ptr ()), argCULong cnt] >>= ownedObject . castPtr
...
arrayWithObjects_count objects cnt =
  do
    cls' <- getRequiredClass "NSArray"
    sendClassMsg cls' (mkSelector "arrayWithObjects:count:") (retPtr retVoid) [argPtr (castPtr (unRawId objects) :: Ptr ()), argCULong cnt] >>= retainedObject . castPtr
...
descriptionWithLocale_indent nsArray  locale level =
    sendMsg nsArray (mkSelector "descriptionWithLocale:indent:") (retPtr retVoid) [argPtr (castPtr (unRawId locale) :: Ptr ()), argCULong level] >>= retainedObject . castPtr
...
objectAtIndexedSubscript nsArray  idx =
    fmap (RawId . castPtr) $ sendMsg nsArray (mkSelector "objectAtIndexedSubscript:") (retPtr retVoid) [argCULong idx]
