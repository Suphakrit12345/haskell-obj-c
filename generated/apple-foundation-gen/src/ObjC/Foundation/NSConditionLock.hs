...
initWithCondition nsConditionLock  condition =
    sendMsg nsConditionLock (mkSelector "initWithCondition:") (retPtr retVoid) [argCLong condition] >>= ownedObject . castPtr

-- | @- lockWhenCondition:@
lockWhenCondition :: IsNSConditionLock nsConditionLock => nsConditionLock -> CLong -> IO ()
lockWhenCondition nsConditionLock  condition =
    sendMsg nsConditionLock (mkSelector "lockWhenCondition:") retVoid [argCLong condition]
...
tryLockWhenCondition nsConditionLock  condition =
    fmap ((/= 0) :: CULong -> Bool) $ sendMsg nsConditionLock (mkSelector "tryLockWhenCondition:") retCULong [argCLong condition]
...
unlockWithCondition nsConditionLock  condition =
    sendMsg nsConditionLock (mkSelector "unlockWithCondition:") retVoid [argCLong condition]
...
lockWhenCondition_beforeDate nsConditionLock  condition limit =
  withObjCPtr limit $ \raw_limit ->
      fmap ((/= 0) :: CULong -> Bool) $ sendMsg nsConditionLock (mkSelector "lockWhenCondition:beforeDate:") retCULong [argCLong condition, argPtr (castPtr raw_limit :: Ptr ())]
