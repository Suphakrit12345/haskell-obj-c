...
timerWithTimeInterval_invocation_repeats ti invocation yesOrNo =
  do
    ...
    withObjCPtr invocation $ \raw_invocation ->
      sendClassMsg cls' (mkSelector "timerWithTimeInterval:invocation:repeats:") (retPtr retVoid) [argCDouble ti, argPtr (castPtr raw_invocation :: Ptr ()), argCULong (if yesOrNo then 1 else 0)] >>= retainedObject . castPtr
...
scheduledTimerWithTimeInterval_invocation_repeats ti invocation yesOrNo =
  do
    cls' <- getRequiredClass "NSTimer"
    withObjCPtr invocation $ \raw_invocation ->
      sendClassMsg cls' (mkSelector "scheduledTimerWithTimeInterval:invocation:repeats:") (retPtr retVoid) [argCDouble ti, argPtr (castPtr raw_invocation :: Ptr ()), argCULong (if yesOrNo then 1 else 0)] >>= retainedObject . castPtr
...
timerWithTimeInterval_target_selector_userInfo_repeats ti aTarget aSelector userInfo yesOrNo =
  do
    cls' <- getRequiredClass "NSTimer"
    sendClassMsg cls' (mkSelector "timerWithTimeInterval:target:selector:userInfo:repeats:") (retPtr retVoid) [argCDouble ti, argPtr (castPtr (unRawId aTarget) :: Ptr ()), argPtr (unSelector aSelector), argPtr (castPtr (unRawId userInfo) :: Ptr ()), argCULong (if yesOrNo then 1 else 0)] >>= retainedObject . castPtr
...
scheduledTimerWithTimeInterval_target_selector_userInfo_repeats ti aTarget aSelector userInfo yesOrNo =
  do
    cls' <- getRequiredClass "NSTimer"
    sendClassMsg cls' (mkSelector "scheduledTimerWithTimeInterval:target:selector:userInfo:repeats:") (retPtr retVoid) [argCDouble ti, argPtr (castPtr (unRawId aTarget) :: Ptr ()), argPtr (unSelector aSelector), argPtr (castPtr (unRawId userInfo) :: Ptr ()), argCULong (if yesOrNo then 1 else 0)] >>= retainedObject . castPtr
...
timerWithTimeInterval_repeats_block interval repeats block =
  do
    cls' <- getRequiredClass "NSTimer"
    sendClassMsg cls' (mkSelector "timerWithTimeInterval:repeats:block:") (retPtr retVoid) [argCDouble interval, argCULong (if repeats then 1 else 0), argPtr (castPtr block :: Ptr ())] >>= retainedObject . castPtr
...
scheduledTimerWithTimeInterval_repeats_block interval repeats block =
  do
    cls' <- getRequiredClass "NSTimer"
    sendClassMsg cls' (mkSelector "scheduledTimerWithTimeInterval:repeats:block:") (retPtr retVoid) [argCDouble interval, argCULong (if repeats then 1 else 0), argPtr (castPtr block :: Ptr ())] >>= retainedObject . castPtr
...
initWithFireDate_interval_repeats_block nsTimer  date interval repeats block =
  withObjCPtr date $ \raw_date ->
      sendMsg nsTimer (mkSelector "initWithFireDate:interval:repeats:block:") (retPtr retVoid) [argPtr (castPtr raw_date :: Ptr ()), argCDouble interval, argCULong (if repeats then 1 else 0), argPtr (castPtr block :: Ptr ())] >>= ownedObject . castPtr
...
initWithFireDate_interval_target_selector_userInfo_repeats nsTimer  date ti t s ui rep =
  withObjCPtr date $ \raw_date ->
      sendMsg nsTimer (mkSelector "initWithFireDate:interval:target:selector:userInfo:repeats:") (retPtr retVoid) [argPtr (castPtr raw_date :: Ptr ()), argCDouble ti, argPtr (castPtr (unRawId t) :: Ptr ()), argPtr (unSelector s), argPtr (castPtr (unRawId ui) :: Ptr ()), argCULong (if rep then 1 else 0)] >>= ownedObject . castPtr
...
setTolerance nsTimer  value =
    sendMsg nsTimer (mkSelector "setTolerance:") retVoid [argCDouble value]
