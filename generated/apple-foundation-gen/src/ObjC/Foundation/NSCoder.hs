...
decodeValueOfObjCType_at_size nsCoder  type_ data_ size =
    sendMsg nsCoder (mkSelector "decodeValueOfObjCType:at:size:") retVoid [argPtr (unConst type_), argPtr data_, argCULong size]
...
encodeArrayOfObjCType_count_at nsCoder  type_ count array =
    sendMsg nsCoder (mkSelector "encodeArrayOfObjCType:count:at:") retVoid [argPtr (unConst type_), argCULong count, argPtr (unConst array)]

-- | @- encodeBytes:length:@
encodeBytes_length :: IsNSCoder nsCoder => nsCoder -> Const (Ptr ()) -> CULong -> IO ()
encodeBytes_length nsCoder  byteaddr length_ =
    sendMsg nsCoder (mkSelector "encodeBytes:length:") retVoid [argPtr (unConst byteaddr), argCULong length_]
...
decodeArrayOfObjCType_count_at nsCoder  itemType count array =
    sendMsg nsCoder (mkSelector "decodeArrayOfObjCType:count:at:") retVoid [argPtr (unConst itemType), argCULong count, argPtr array]
...
encodeInt_forKey nsCoder  value key =
  withObjCPtr key $ \raw_key ->
      sendMsg nsCoder (mkSelector "encodeInt:forKey:") retVoid [argCInt value, argPtr (castPtr raw_key :: Ptr ())]

-- | @- encodeInt32:forKey:@
encodeInt32_forKey :: (IsNSCoder nsCoder, IsNSString key) => nsCoder -> CInt -> key -> IO ()
encodeInt32_forKey nsCoder  value key =
  withObjCPtr key $ \raw_key ->
      sendMsg nsCoder (mkSelector "encodeInt32:forKey:") retVoid [argCInt value, argPtr (castPtr raw_key :: Ptr ())]

-- | @- encodeInt64:forKey:@
encodeInt64_forKey :: (IsNSCoder nsCoder, IsNSString key) => nsCoder -> CLong -> key -> IO ()
encodeInt64_forKey nsCoder  value key =
  withObjCPtr key $ \raw_key ->
      sendMsg nsCoder (mkSelector "encodeInt64:forKey:") retVoid [argCLong value, argPtr (castPtr raw_key :: Ptr ())]

-- | @- encodeFloat:forKey:@
encodeFloat_forKey :: (IsNSCoder nsCoder, IsNSString key) => nsCoder -> CFloat -> key -> IO ()
encodeFloat_forKey nsCoder  value key =
  withObjCPtr key $ \raw_key ->
      sendMsg nsCoder (mkSelector "encodeFloat:forKey:") retVoid [argCFloat value, argPtr (castPtr raw_key :: Ptr ())]

-- | @- encodeDouble:forKey:@
encodeDouble_forKey :: (IsNSCoder nsCoder, IsNSString key) => nsCoder -> CDouble -> key -> IO ()
encodeDouble_forKey nsCoder  value key =
  withObjCPtr key $ \raw_key ->
      sendMsg nsCoder (mkSelector "encodeDouble:forKey:") retVoid [argCDouble value, argPtr (castPtr raw_key :: Ptr ())]
...
encodeBytes_length_forKey nsCoder  bytes length_ key =
  withObjCPtr key $ \raw_key ->
      sendMsg nsCoder (mkSelector "encodeBytes:length:forKey:") retVoid [argPtr (unConst bytes), argCULong length_, argPtr (castPtr raw_key :: Ptr ())]
...
decodeBytesWithMinimumLength nsCoder  length_ =
    fmap castPtr $ sendMsg nsCoder (mkSelector "decodeBytesWithMinimumLength:") (retPtr retVoid) [argCULong length_]
...
decodeBytesForKey_minimumLength nsCoder  key length_ =
  withObjCPtr key $ \raw_key ->
      fmap Const $ fmap castPtr $ sendMsg nsCoder (mkSelector "decodeBytesForKey:minimumLength:") (retPtr retVoid) [argPtr (castPtr raw_key :: Ptr ()), argCULong length_]

-- | @- encodeInteger:forKey:@
encodeInteger_forKey :: (IsNSCoder nsCoder, IsNSString key) => nsCoder -> CLong -> key -> IO ()
encodeInteger_forKey nsCoder  value key =
  withObjCPtr key $ \raw_key ->
      sendMsg nsCoder (mkSelector "encodeInteger:forKey:") retVoid [argCLong value, argPtr (castPtr raw_key :: Ptr ())]
