...
objectAtIndex nsOrderedSet  idx =
    fmap (RawId . castPtr) $ sendMsg nsOrderedSet (mkSelector "objectAtIndex:") (retPtr retVoid) [argCULong idx]
...
initWithObjects_count nsOrderedSet  objects cnt =
    sendMsg nsOrderedSet (mkSelector "initWithObjects:count:") (retPtr retVoid) [argPtr (castPtr (unRawId objects) :: Ptr ()), argCULong cnt] >>= ownedObject . castPtr
...
orderedSetWithObjects_count objects cnt =
  do
    cls' <- getRequiredClass "NSOrderedSet"
    sendClassMsg cls' (mkSelector "orderedSetWithObjects:count:") (retPtr retVoid) [argPtr (castPtr (unRawId objects) :: Ptr ()), argCULong cnt] >>= retainedObject . castPtr
...
objectAtIndexedSubscript nsOrderedSet  idx =
    fmap (RawId . castPtr) $ sendMsg nsOrderedSet (mkSelector "objectAtIndexedSubscript:") (retPtr retVoid) [argCULong idx]
...
descriptionWithLocale_indent nsOrderedSet  locale level =
    sendMsg nsOrderedSet (mkSelector "descriptionWithLocale:indent:") (retPtr retVoid) [argPtr (castPtr (unRawId locale) :: Ptr ()), argCULong level] >>= retainedObject . castPtr
