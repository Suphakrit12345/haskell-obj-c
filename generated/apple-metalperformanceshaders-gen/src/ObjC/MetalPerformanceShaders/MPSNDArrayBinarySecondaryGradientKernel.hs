...
initWithDevice_sourceCount_sourceGradientIndex mpsndArrayBinarySecondaryGradientKernel  device count sourceGradientIndex =
    sendMsg mpsndArrayBinarySecondaryGradientKernel (mkSelector "initWithDevice:sourceCount:sourceGradientIndex:") (retPtr retVoid) [argPtr (castPtr (unRawId device) :: Ptr ()), argCULong count, argCULong sourceGradientIndex] >>= ownedObject . castPtr
