...
initWithDevice_transpose_rows_columns_alpha_beta mpsMatrixVectorMultiplication  device transpose rows columns alpha beta =
    sendMsg mpsMatrixVectorMultiplication (mkSelector "initWithDevice:transpose:rows:columns:alpha:beta:") (retPtr retVoid) [argPtr (castPtr (unRawId device) :: Ptr ()), argCULong (if transpose then 1 else 0), argCULong rows, argCULong columns, argCDouble alpha, argCDouble beta] >>= ownedObject . castPtr
...
initWithDevice_rows_columns mpsMatrixVectorMultiplication  device rows columns =
    sendMsg mpsMatrixVectorMultiplication (mkSelector "initWithDevice:rows:columns:") (retPtr retVoid) [argPtr (castPtr (unRawId device) :: Ptr ()), argCULong rows, argCULong columns] >>= ownedObject . castPtr
