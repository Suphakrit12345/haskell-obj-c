...
initWithDevice_sourceCount_sourceGradientIndex mpsndArrayBinaryPrimaryGradientKernel  device count sourceGradientIndex =
    sendMsg mpsndArrayBinaryPrimaryGradientKernel (mkSelector "initWithDevice:sourceCount:sourceGradientIndex:") (retPtr retVoid) [argPtr (castPtr (unRawId device) :: Ptr ()), argCULong count, argCULong sourceGradientIndex] >>= ownedObject . castPtr
