...
nodeWithSource_keepProbability source keepProbability =
  do
    cls' <- getRequiredClass "MPSCNNDropoutNode"
    withObjCPtr source $ \raw_source ->
      sendClassMsg cls' (mkSelector "nodeWithSource:keepProbability:") (retPtr retVoid) [argPtr (castPtr raw_source :: Ptr ()), argCFloat keepProbability] >>= retainedObject . castPtr
...
initWithSource_keepProbability mpscnnDropoutNode  source keepProbability =
  withObjCPtr source $ \raw_source ->
      sendMsg mpscnnDropoutNode (mkSelector "initWithSource:keepProbability:") (retPtr retVoid) [argPtr (castPtr raw_source :: Ptr ()), argCFloat keepProbability] >>= ownedObject . castPtr
