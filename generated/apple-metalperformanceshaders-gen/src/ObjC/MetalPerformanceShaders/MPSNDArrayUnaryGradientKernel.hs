...
initWithDevice_sourceCount_sourceGradientIndex mpsndArrayUnaryGradientKernel  device count sourceGradientIndex =
    sendMsg mpsndArrayUnaryGradientKernel (mkSelector "initWithDevice:sourceCount:sourceGradientIndex:") (retPtr retVoid) [argPtr (castPtr (unRawId device) :: Ptr ()), argCULong count, argCULong sourceGradientIndex] >>= ownedObject . castPtr
