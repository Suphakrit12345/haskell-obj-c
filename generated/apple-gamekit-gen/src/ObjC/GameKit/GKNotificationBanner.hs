...
showBannerWithTitle_message_duration_completionHandler title message duration completionHandler =
  do
    ...
    withObjCPtr title $ \raw_title ->
      withObjCPtr message $ \raw_message ->
        sendClassMsg cls' (mkSelector "showBannerWithTitle:message:duration:completionHandler:") retVoid [argPtr (castPtr raw_title :: Ptr ()), argPtr (castPtr raw_message :: Ptr ()), argCDouble duration, argPtr (castPtr completionHandler :: Ptr ())]
