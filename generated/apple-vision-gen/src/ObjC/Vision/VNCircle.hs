...
initWithCenter_radius vnCircle  center radius =
  withObjCPtr center $ \raw_center ->
      sendMsg vnCircle (mkSelector "initWithCenter:radius:") (retPtr retVoid) [argPtr (castPtr raw_center :: Ptr ()), argCDouble radius] >>= ownedObject . castPtr
...
initWithCenter_diameter vnCircle  center diameter =
  withObjCPtr center $ \raw_center ->
      sendMsg vnCircle (mkSelector "initWithCenter:diameter:") (retPtr retVoid) [argPtr (castPtr raw_center :: Ptr ()), argCDouble diameter] >>= ownedObject . castPtr
...
containsPoint_inCircumferentialRingOfWidth vnCircle  point ringWidth =
  withObjCPtr point $ \raw_point ->
      fmap ((/= 0) :: CULong -> Bool) $ sendMsg vnCircle (mkSelector "containsPoint:inCircumferentialRingOfWidth:") retCULong [argPtr (castPtr raw_point :: Ptr ()), argCDouble ringWidth]
